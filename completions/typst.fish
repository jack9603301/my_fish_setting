...
complete -c typst -n "__fish_typst_needs_command" -f -a "query" -d 'Processes an input file to extract provided metadata (deprecated, use `eval` instead)'
complete -c typst -n "__fish_typst_needs_command" -f -a "eval" -d 'Evaluates a piece of Typst code, optionally in the context of a document'
...
complete -c typst -n "__fish_typst_using_subcommand eval" -l in -d 'A file in whose context to evaluate the code. Can be used to introspect the document. Use `-` to read input from stdin' -r -F
complete -c typst -n "__fish_typst_using_subcommand eval" -l target -d 'The target to compile for' -r -f -a "paged\t'PDF and image formats'
html\t'HTML'"
complete -c typst -n "__fish_typst_using_subcommand eval" -l format -d 'The format to serialize in' -r -f -a "json\t''
yaml\t''"
complete -c typst -n "__fish_typst_using_subcommand eval" -l root -d 'Configures the project root (for absolute paths)' -r -F
complete -c typst -n "__fish_typst_using_subcommand eval" -l input -d 'Add a string key-value pair visible through `sys.inputs`' -r
complete -c typst -n "__fish_typst_using_subcommand eval" -l font-path -d 'Adds additional directories that are recursively searched for fonts' -r -F
complete -c typst -n "__fish_typst_using_subcommand eval" -l package-path -d 'Custom path to local packages, defaults to system-dependent location' -r -F
complete -c typst -n "__fish_typst_using_subcommand eval" -l package-cache-path -d 'Custom path to package cache, defaults to system-dependent location' -r -F
complete -c typst -n "__fish_typst_using_subcommand eval" -l creation-timestamp -d 'The document\'s creation date formatted as a UNIX timestamp' -r
complete -c typst -n "__fish_typst_using_subcommand eval" -s j -l jobs -d 'Number of parallel jobs spawned during compilation. Defaults to number of CPUs. Setting it to 1 disables parallelism' -r
complete -c typst -n "__fish_typst_using_subcommand eval" -l features -d 'Enables in-development features that may be changed or removed at any time' -r -f -a "html\t''
a11y-extras\t''"
complete -c typst -n "__fish_typst_using_subcommand eval" -l diagnostic-format -d 'The format to emit diagnostics in' -r -f -a "human\t''
short\t''"
complete -c typst -n "__fish_typst_using_subcommand eval" -l pretty -d 'Whether to pretty-print the serialized output'
complete -c typst -n "__fish_typst_using_subcommand eval" -l ignore-system-fonts -d 'Ensures system fonts won\'t be searched, unless explicitly included via `--font-path`'
complete -c typst -n "__fish_typst_using_subcommand eval" -l ignore-embedded-fonts -d 'Ensures fonts embedded into Typst won\'t be considered'
complete -c typst -n "__fish_typst_using_subcommand eval" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "compile" -d 'Compiles an input file into a supported output format'
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "watch" -d 'Watches an input file and recompiles on changes'
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "init" -d 'Initializes a new project from a template'
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "query" -d 'Processes an input file to extract provided metadata (deprecated, use `eval` instead)'
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "eval" -d 'Evaluates a piece of Typst code, optionally in the context of a document'
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "fonts" -d 'Lists all discovered fonts in system and custom font paths'
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "update" -d 'Self update the Typst CLI'
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "completions" -d 'Generates shell completion scripts'
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "info" -d 'Displays debugging information about Typst'
complete -c typst -n "__fish_typst_using_subcommand help; and not __fish_seen_subcommand_from compile watch init query eval fonts update completions info help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
